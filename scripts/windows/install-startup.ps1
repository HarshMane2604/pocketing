...
$runner = Join-Path $PSScriptRoot 'start-memory-inbox.ps1'
$launcher = Join-Path $PSScriptRoot 'open-memory-inbox.ps1'
...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Description 'Runs the local Memory Inbox API, PWA, WebSocket, and messaging bridges.' -Force | Out-Null
...
foreach ($shortcutPath in @(
    (Join-Path $desktop 'Memory Inbox.lnk'),
    (Join-Path $startup 'Memory Inbox.lnk')
)) {
...
Write-Host 'Memory Inbox is installed. It will run and open automatically at Windows login.'
Write-Host 'Desktop shortcut created: Memory Inbox'
