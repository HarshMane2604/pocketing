...
import { CheckIcon, CircleIcon, CopyIcon, FolderIcon, PauseIcon, PinIcon, ThreadIcon, TrashIcon } from '@/components/Icons';
...
export function NoteRow({ note, busy, onUpdate, onDelete, onOpenThread, dragHandleProps, isDragging }: NoteRowProps) {
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger>
        <article className={cls} {...dragHandleProps}>
          <button
            type="button"
            onClick={() => onUpdate(note, { is_done: !note.is_done })}
            disabled={busy}
      ...
      <ContextMenuContent>
        {(!note.attachments || note.attachments.length === 0) && (
          <ContextMenuItem onClick={() => onUpdate(note, { is_on_hold: !note.is_on_hold })}>
            <PauseIcon size={14} className="mr-2" />
            {note.is_on_hold ? 'Resume' : 'Move to On Hold'}
          </ContextMenuItem>
        )}
        {note.attachments && note.attachments.length > 0 && (
          <ContextMenuItem onClick={() => onUpdate(note, { kind: 'file' })}>
            <FolderIcon size={14} className="mr-2" />
            Move to Files
          </ContextMenuItem>
        )}
        <ContextMenuItem onClick={() => onUpdate(note, { is_pinned: !note.is_pinned })}>
          <PinIcon size={14} className="mr-2" />
          {note.is_pinned ? 'Unpin Note' : 'Pin Note'}
        </ContextMenuItem>
        <ContextMenuItem onClick={() => onUpdate(note, { is_done: !note.is_done })}>
          <CheckIcon size={14} className="mr-2" />
          {note.is_done ? 'Mark Not Done' : 'Mark Done'}
        </ContextMenuItem>
        <ContextMenuSeparator />
        <ContextMenuItem onClick={() => void copyNote()}>
          <CopyIcon size={14} className="mr-2" />
          {copied ? 'Copied' : 'Copy Text'}
        </ContextMenuItem>
        {onOpenThread && (
          <ContextMenuItem onClick={() => onOpenThread(note)}>
            <ThreadIcon size={14} className="mr-2" />
            Open Thread
          </ContextMenuItem>
        )}
        <ContextMenuSeparator />
        <ContextMenuItem variant="destructive" onClick={() => onDelete(note)}>
